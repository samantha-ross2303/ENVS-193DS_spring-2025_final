...
nest_boxes_clean <- nest_boxes |>  # starting with the drought_exp object
  # cleaning column names
  clean_names() |> 
  # renaming columns
  rename(
  ...
  mutate(
    ...
    swift_parrot = factor(swift_parrot, levels = c(0, 1))
  )
...
par(mfrow = c(2,2))
plot(model0)
plot(model1) # residuals look ok for best model
plot(model2)
plot(model3)
